...
class FlatsList {
  constructor(config, filter) {
    ...
    this.card_bottom_labels = filter.card_bottom_labels || [];
    this.init();
  }
  ...
  createElem(flat, favourites) {
    ...
    if (this.filter.viewType === 'card') {
      ...
      return `
         ${Card(this.i18n, flat, this.favouritesIds$, this.show_prices, atrEnter, atrLeave, false, this.card_bottom_labels)}
      `;
    }
    return tr;
  }
