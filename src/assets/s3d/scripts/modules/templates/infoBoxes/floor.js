...
function Floor(i18n, data) {
  ...
  const $price =
    show_prices && properties._price
      ? `
    <div class="text-style-3-d-fonts-1920-h-1 s3d-infoBox__title">
      ${currency === '$' ? `${i18n.t(`currency_label`, '')} ${numberWithCommas(properties._price.value_raw)}` : `${numberWithCommas(properties._price.value_raw)} ${i18n.t(`currency_label`, '')}`}
    </div>
  `
      : '';
  const $price_m2 =
    show_prices && properties.price_m2
      ? `
    <div class="s3d-infoBox__flat__block" style="margin-top: 8px">
  ...
  const buttonHtml = isDesktop() && isNotDesktopTouchMode()
  ? ''
  : `
  ...
  return `
    <div class="s3d-infoBox__flat">
        ...
        <div class="s3d-infoBox__flat-bottom">
          ${$price || $price_m2 ? `<div class="s3d-infoBox__flat__alert-title">
