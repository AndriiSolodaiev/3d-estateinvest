...
class Plannings {
  ...
  init() {
    ...
    $('.js-s3d-pl__list').on('click', '.js-s3d-card', event => {
      if (event.target.closest('.js-s3d-add__favourite') || event.target.closest('[data-show-flat-in-flyby]')) {
        return;
      }
    ...
    flats.forEach((id, index) => {
      if (index >= this.currentShowAmount && index < this.currentShowAmount + amount) {
        wrap.insertAdjacentHTML(
          'beforeend',
          Card(this.i18n, this.getFlat(id), this.favouritesIds$, this.show_prices, undefined, undefined, false, this.card_bottom_labels),
        );
      }
    });
